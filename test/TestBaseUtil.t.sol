...
import { IDittoEntryPoint } from "src/entrypoint/interfaces/IDittoEntryPoint.sol";
import { DittoEntryPoint } from "src/entrypoint/core/DittoEntryPoint.sol";
...
contract TestBaseUtil is BootstrapUtil, Test {
    ...
    Vm.Wallet public OWNER_ENTRYPOINT;

    function setUp() public virtual {
        // Set up EntryPoint
        OWNER_ENTRYPOINT = newWallet("OWNER_ENTRYPOINT");

        vm.prank(OWNER_ENTRYPOINT.addr);
        entrypoint = new DittoEntryPoint();
        
        vm.etch(address(ENTRYPOINT_ADDR), address(entrypoint).code);
        
        entrypoint = IDittoEntryPoint(0x0000000071727De22E5E9d8BAf0edAc6f37da032);
